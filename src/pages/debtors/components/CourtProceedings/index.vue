  ...
  <div class="debtors">
   <!--  <div class="main__content-header">
    ...
    <ur-table
      ...
      :displayRows="debtorsCourtProceedings"
      :allColumns="courtProceedingsTableColumns"
      @columnsChange="updateColumns($event)"
      :itemsPerPage="debtorsCourtProceedingsInPage"
    ...
    >
      <template v-slot:controls>
        ...
        <!-- <transition name="fade">
          <div
            v-show="checkedDebtorsCount >= 1 && allDebtorsInPageCount > 0 && checkedDebtorsCount !== allDebtorsInPageCount"
            class="table__nav-action"
            @click="checkAllClonesDebtors()"
          >
            <span>Выбрать всех на странице (+{{ allDebtorsInPageCount - checkedDebtorsCount }})</span>
          </div>
        </transition> -->
      ...
      <template v-slot:head(checked)="{ row, i }">
        <div style="display: flex; justify-content: center">
          <ur-checkbox
            :checked="checkedDebtorsCount === debtorsCountInCurrentPage"
            @change="handlerHeaderCheckbox"
          />
          <!-- <span style="border: 1px solid red" @click.stop="handlerHeaderCheckbox">{{ checkedDebtorsCount === debtorsCountInCurrentPage }}</span> -->
        </div>
      </template>
      ...
      <template v-slot:cell(Adres)="{ row, i }">
        <span class="table__col-user" v-if="!row.clean_address">
          <!-- {{ row['ListResidents'][0].LocationDebter }}, {{ row['Adres'] }} -->
...
<!--            <tr class="table__row table__row&#45;&#45;is-total">-->
<!--              <td v-for="(item, index) in courtProceedingsTableColumnsVisible">-->
<!--                <span class="table__col-user" v-if="index === 0">Итого:</span>-->
<!--                <span class="table__col-user" style="opacity: 0; width: 100%" v-if="!item.isSum" ></span>-->
<!--                <span class="table__col-user" v-if="item.isSum">-->
<!--                    {{ debtorsData[item.name] | sepnum(2) }} ₽-->
<!--                   </span>-->
<!--              </td>-->
<!--            </tr>-->
<!--            </tbody>-->
...
<!--        </div>-->
      ...
      <template v-slot:contextMenu>
        <div class="context-menu" :class="{'is-active' : contextMenuOpenFlag }" ref="contextMenu">
          ...
          <div class="context-menu__item" :disabled="!checkedDebtorsCount.length" @click="printItem(); contextMenuOpenFlag = false">
            <span>Печать</span>
          </div>
        </div>
      </template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions([
        'saveCourtProceedingsTableColumns',
        'getDebtorsCourtProceedingsList',
        'updateCourtProceedingsList',
        'combinedRequests',
        'setPopupComponent',
        'setCurrentPage',
        'setPopupState',
      ...
      ...mapMutations([
       ...
       uncheckAllClonesDebtors () {
         this.uncheckAllDebtors({module: 1})
      },
      parseStatus(status) {
      ...
      async updateColumns ({ itemsPerPage, items }) {
        await this.saveCourtProceedingsTableColumns({ items, itemsPerPage });
        this.updateCourtProceedingsList();
      },
      /**
       * Обработчик события check в заголовке таблицы задолжников
       */
      handlerHeaderCheckbox () {
        // this.checkedDebtorsCount === parseInt(this.debtorsCourtProceedingsInPage) ? this.uncheckAllDebtors() : this.checkAllDebtors()
        this.checkedDebtorsCount === this.allDebtorsInPageCount ? this.uncheckAllDebtors() : this.checkAllDebtors()
      },
      /**
      ...
      sortHandler(e) {
        ...
        switch (e.sortDirection) {
          ...
          case 'none':
            items = this.debtorsCourtProceedingsInner.sort(() => {
              return 0.5 - Math.random();
            })
            break;
        }
        this.debtorsCourtProceedingsInner = items;
      },
      // поиск
      toggleSearchPanel() {
      ...
      runActionFromIconHandler (e) {
        ...
        //} else if (e.action === 'printDigitalSignature') {
          ...
          if(e.action === 'printSetOfCharges') {
            this.setPopupComponent({ component: 'debtor-set-of-charges-print-modal', params: { checkedDebtors: this.checkedDebtors } });
            // this.modals.SetOfChargesModal = true
          } else if(e.action === 'printPaymentForm') {
            this.setPopupComponent({component: 'PrintTemplatePopup', params: {checkedDebtors: this.checkedDebtors}})
            ...
            this.setPopupComponent({ component: 'notify-modal', params: { checkDebtors: this.checkedDebtors, mode: 'sms' }})
          } else if (e.action === 'sendVoiceMsg') {
            this.setPopupComponent({ component: 'notify-modal', params: { checkDebtors: this.checkedDebtors, mode: 'voice' }})
          } else {
            this.printModal = true;
          }
     ...
     async getApplicationDischarge() {
      ...
      async printDigitalSignature(format) {
        let cloned = cloneDeep(this.checkedDebtors);
       ...
       */
      printItem() {
        const debtor = this.debtorsCourtProceedings[this.currentOpen];
        this.printStatementsJudicalModule([debtor]);
      },
      formAnApplication() {
        if(this.checkedDebtors.length === 0) return;
        const payload = {};
        this.checkedDebtors.forEach((d, index) => payload[index] = d.RashSchet)
    ...
    computed: {
      ...mapGetters([
        ...
        'courtProceedingsTableColumnsVisible',
        'debtorsCourtProceedings',
        'debtorsCourtProceedingsInPage', // количество задолжников на странице В НАСТРОЙКАХ
        'debtorsCountInCurrentPage', // на текущей странице
        'checkedDebtorsCount', // количество отмеченных чекбоксов
        'pages',
        'debtorsData',
        'initialized',
        'getDefaultCompany',
      ...
      allDebtorsInPageCount () {
        return this.debtorsCourtProceedingsInner.length
      },
      /**
       ...
       */
      formattedColumns () {
        return this.courtProceedingsTableColumnsVisible.map(i => {
          return {
            title: i.alias,
            key: i.name,
