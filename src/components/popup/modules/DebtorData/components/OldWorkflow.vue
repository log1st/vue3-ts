...
<!--    <div class="d-data__content-row">-->
<!--      <div class="btn btn-primary" @click="refreshFilesList">Обновить</div>-->
<!--      <div class="btn btn-white" @click="$store.dispatch('removeAll')">Удалить</div>-->
<!--    </div>-->
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      updateFilesTable() {
        ...
        // debugger
        this.files = Object.values(debtor.AllFileDebt).reverse();
      },
      changeFile (event) {
        ...
        files.forEach(async file => {
          ...
          this.saveDebtorFile(data)
            .then(res => {
              ...
              this.addHistoryNote({
                Names: 'Загрузка документов (Судебная практика - Документооборот)',
                DataTime: new Date(),
                Status: 'Загружено',
                PaymentAccount: parseInt(this.params.RashSchet)
              }).then(() => {
      ...
      runActions (e) {
        ...
        switch (e) {
          ...
          case 0:
            this.downloadFile({ PaymentAccount: this.params.RashSchet, File: file.FileName }).then(res => {
