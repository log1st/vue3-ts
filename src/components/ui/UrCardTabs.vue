    ...
    <div class="cart__tabs">
      ...
      >
        <div class="cart__tab-decor is-left">
          <icon-base width="14" height="8" :iconColor="active == i ? '#495cff' : '#F2F6FC'"><icon-decor /></icon-base>
        </div>
        <span v-html="item.title"></span>
          <!-- 
               Костыль конечно но таков дизайн 
               TODO: добавить params для Icon
           -->
          <icon-base style="margin-left: 0.5em" 
          :width="item.width" 
          :height="item.height" 
      ...
      <transition name="fade">
        <div v-if="buttons" class="cart__tabs-btn">
          <div v-for="(item, i) in buttons.array" class="btn btn-icon" :key="'crdfh' + i">
            <icon-base :width="item.width" :height="item.height" iconColor="#000">
                <component :is="'icon-' + item.icon" />
            </icon-base>
          </div>
        </div>
