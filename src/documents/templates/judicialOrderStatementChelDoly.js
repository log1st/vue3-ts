...
const Component = Vue.extend({
    props: ['company', 'debtor', 'services', 'ecp', 'documentsList', 'equityРolder', 'court'],
    template: `
    <div>
        <table width="716" cellpadding="7" cellspacing="0">
            <tbody>
                ...
                <tr>
                    ...
                    <td style="border: none; padding: 0; width: 50%">{{ court.Address }}</td>
                </tr>
                <tr>
                ...
                <tr>
                    <td style="border: none; padding: 0; text-align: right; width: 50%">Адрес:</td>
                    <td style="border: none; padding: 0; width: 50%">
                        <p style="margin: 0; text-align: left">{{ debtor.Postal }}, {{ debtor.Adres }}</p>
                    </td>
                </tr>
                <tr>
        ...
        <p>
            <b> Согласно Справке (п.3 Приложение) {{ equityРolder.Doly }} часть жилого помещения, находящегося по адресу: </b>
            {{ debtor.Adres }} принадлежит <b> {{ equityРolder.FIO }}, на праве собственности. </b>
        </p>
        ...
        <table width="716" cellpadding="7" cellspacing="0">
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align: center"><b>с 01.01.2020г. по 31.12.2020г</b></td>
                    <td style="text-align: center"><b>469,05</b></td>
                    <td style="text-align: center"><b>81,61</b></td>
                </tr>
            </tbody>
        </table>
        ...
        <p>
            В соответствии со сверкой л/с по указанному выше жилому помещению имеется задолженность за период с <b>{{ dateStart }} по {{ dateEnd }}</b> 
            в размере <b>{{ debtor.TotalDebt | currencyHumanReadableFilter }}.</b> ({{ rubles(debtor.TotalDebt) }}). 
            Сумма задолженности <b>{{ equityРolder.GenitiveFIO || 'check api' }}</b>, соразмерно <b>{{ equityРolder.Doly }}</b> доли 
            составляет {{ calcDoly(equityРolder.Doly, debtor.TotalDebt) | currencyHumanReadableFilter }} ({{ rubles(calcDoly(equityРolder.Doly, debtor.TotalDebt)) }})
        </p>
        ...
        <p style="margin:0; text-align: justify; font-size: 11pt">
            <span style="color: white">____</span>
            <b>1. Вынести судебный приказ о взыскании {{ Object.values(debtor.ListResidents).length > 1 ? 'в солидарном порядке ' : '' }} с 
            <template v-for="(item, index) in debtor.ListResidents">
                <span><b>{{ item.FioDebtGenitive || '_____________' }}</b></span>, 
                <span>{{ item.DateBirthDebt || '_____________' }} г.р.</span>
                {{ debtor.ListResidents[Number(index) + 1] ? ',' : ' ' }}
            </template>
            в пользу {{ company.ShortNameOfTheOrganization }}: </b><br>
            - задолженности по оплате коммунальной услуги по обращению с ТКО за период с {{ dateStart }} по {{ dateEnd }} соразмерно <b>{{ equityРolder.Doly }}</b> доликоторая составляет {{ calcDoly(equityРolder.Doly, debtor.TotalDebt) | currencyHumanReadableFilter }} ({{ rubles(calcDoly(equityРolder.Doly, debtor.TotalDebt)) }}); <br>
    ...
    async createTemplate (props) {
        ...
        const vm = new Component({
            propsData: {
                ...
                ecp: props.ecp,
                equityРolder: props.equityРolder 
            }
        })
        // debugger
        console.log(props)
